...
namespace Tensors
{
    namespace Small
    {
        ...
        public:
            ...
            explicit SymmetricMatrix( const Real init )
            {
                Fill(init);
            }
            ...
            force_inline void Fill( const Real init )
            {
            ...
            force_inline const Real & operator()( const Int i, const Int j ) const
            {
                return A[i][j];
            }
            
            friend SymmetricMatrix operator+( const SymmetricMatrix & x, const SymmetricMatrix & y )
            {
                SymmetricMatrix z;
            ...
            void operator+=( const SymmetricMatrix & B )
            {
                for( Int i = 0; i < AmbDim; ++i )
                {
                    for( Int j = i; j < AmbDim; ++j )
            ...
            force_inline void Write( Real * target ) const
            {
                copy_buffer( &A[0][0], target, AmbDim * AmbDim );
            }
            
            force_inline void Read( Real const * const source )
            {
            ...
            //            #pragma omp declare reduction( + : SymmetricMatrix : omp_out+=omp_in )
        ...
        public:
            ...
            {
                return "SymmetricMatrix<"+std::to_string(AmbDim)+","+TypeName<Real>::Get()+","+TypeName<Int>::Get()+">";
            }
